...
Class gettingClientsInfo {
    ...
    // ќпределение параметров. —сылку (куда будет делатьс€ запрос), какие заголовки будут у этого запроса, задаем, что запрос должен быть в формате POST и передаем параметры этого запроса в виде массива 'ключ' => 'значение'. 
    ...
    curl_setopt_array($curl, [
        ...
        CURLOPT_URL => 'https://ws2.tvojdoktor.ru/Monitoring/hs/Web/GetPhone', //урл дл€ запроса
        CURLOPT_PORT => 333,
        CURLOPT_POST => 1, //метод POST
        CURLOPT_USERPWD => "robot1C:Sa1ch34ee9",
        CURLOPT_POSTFIELDS => $paramArr
    ]);
    // ќтправл€ем запроса и сохран€ем его в $res
    $res = curl_exec($curl);
    $clientInfo = simplexml_load_string($res);
    $clientInfoArr = array();
    // «акрываем запрос и удал€ем инициализацию $curl
    $cardNumber = "";
