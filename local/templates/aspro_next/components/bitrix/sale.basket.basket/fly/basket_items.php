				...
				<tr data-id="total_buttons" class="bottom_btn">
					<td>
						<div class="basket_close">
							<span class="btn btn-default white grey close"><span><?=GetMessage("SALE_BACK")?></span></span>
					...
					<?if ($arParams["AJAX_MODE_CUSTOM"]!="Y"):?>
						<td>
							<div class="basket_update clearfix">
								<button type="submit"  name="BasketRefresh" class="btn btn-default white grey refresh"><span><?=GetMessage("SALE_REFRESH")?></span></button>
					...
					<?if ($arParams["SHOW_FULL_ORDER_BUTTON"]=="Y" && !$arError["ERROR"]):?>
						<td>
							<div class="basket_checkout clearfix">
								<a data-href="<?=$arParams["PATH_TO_ORDER"];?>" href="<?=$arParams["PATH_TO_ORDER"];?>" class="btn btn-default white checkout"><span><?=GetMessage("SALE_ORDER")?></span></a>
						...
						<?if(!$arError["ERROR"]){?>
							<?if (\Bitrix\Main\Config\Option::get("aspro.next", "SHOW_ONECLICKBUY_ON_BASKET_PAGE", "Y") == "Y"):?>
								<div class="basket_fast_order clearfix">
									<a onclick="oneClickBuyBasket()" class="btn btn-default fast_order"><span><?=GetMessage("SALE_FAST_ORDER")?></span></a>
